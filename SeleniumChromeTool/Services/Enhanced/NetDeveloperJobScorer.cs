...
/// </summary>
public class NetDeveloperJobScorer
{
    private readonly ILogger<NetDeveloperJobScorer> _logger;

    public NetDeveloperJobScorer(ILogger<NetDeveloperJobScorer> logger)
    {
        _logger = logger;
    }
    ...
    public JobScoringResult CalculateEnhancedMatchScore(EnhancedJobListing job, NetDeveloperScoringProfile profile)
    {
        ...
        try
        {
            ...
            _logger.LogDebug($"Scored job '{job.Title}' at '{job.Company}': {result.TotalScore}% " +
        ...
        {
            _logger.LogWarning($"Error calculating job score for '{job.Title}': {ex.Message}");
