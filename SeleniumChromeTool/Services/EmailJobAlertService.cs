...
public class EmailJobAlertService
{
    private readonly ILogger<EmailJobAlertService> _logger;
    private readonly string _imapServer;
    private readonly int _imapPort;
    private readonly string _username;
    private readonly string _password;

    public EmailJobAlertService(ILogger<EmailJobAlertService> logger, IConfiguration configuration)
    {
        _logger = logger;
    ...
    public async Task<List<EnhancedJobListing>> GetJobAlertsAsync(int daysBack = 7)
    {
        ...
        try
        {
            ...
            _logger.LogInformation($"Found {messageIds.Count} job alert emails in last {daysBack} days");
            
            // Add debug logging to see what emails we're finding
            if (messageIds.Count == 0)
            {
                _logger.LogWarning($"No emails found matching job alert criteria in last {daysBack} days");
                ...
                _logger.LogInformation($"Total emails in last {daysBack} days: {recentEmails.Count}");
                ...
                foreach (UniqueId id in recentEmails.Take(5))
                {
                    ...
                    {
                        _logger.LogWarning($"Error checking recent email: {ex.Message}");
                    }
                }
            }
            ...
            foreach (UniqueId messageId in messageIds.Take(50)) // Limit to recent 50 emails
            {
                ...
                {
                    _logger.LogWarning($"Error parsing email {messageId}: {ex.Message}");
                }
            }
            
            await client.DisconnectAsync(true);
            
            _logger.LogInformation($"Successfully parsed {jobs.Count} jobs from email alerts");
            return jobs;
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error retrieving job alert emails: {ex.Message}");
            return jobs;
        }
    }
    ...
    private async Task<List<EnhancedJobListing>> ParseJobAlertEmail(MimeMessage message)
    {
        ...
        try
        {
            ...
            _logger.LogDebug($"Processing email from: {sender}, Subject: {subject}");
        ...
        {
            _logger.LogWarning($"Error parsing email: {ex.Message}");
            return jobs;
        }
    }
    ...
    private async Task<List<EnhancedJobListing>> ParseLinkedInJobAlert(string subject, string body, DateTime emailDate)
    {
        ...
        try
        {
            ...
            _logger.LogDebug($"Parsed {jobs.Count} jobs from LinkedIn email alert");
            return jobs;
        }
        catch (Exception ex)
        {
            _logger.LogWarning($"Error parsing LinkedIn email: {ex.Message}");
            return jobs;
        }
    }
    ...
    private async Task<List<EnhancedJobListing>> ParseGlassdoorJobAlert(string subject, string body, DateTime emailDate)
    {
        ...
        try
        {
            _logger.LogInformation($"Parsing Glassdoor email with subject: {subject}");
            _logger.LogDebug($"Email body length: {body.Length} characters");
            ...
            foreach (string pattern in patterns)
            {
                ...
                if (jobMatches.Count > 0)
                {
                    foreach (Match match in jobMatches)
                    {
                        ...
                        jobs.Add(job);
                        _logger.LogInformation($"Extracted job: {job.Title} at {job.Company}");
                    }
            ...
            if (jobs.Count > 0)
            {
                _logger.LogInformation($"Attempting to enhance {jobs.Count} Glassdoor jobs with detailed information");
                for (var i = 0; i < jobs.Count; i++)
                {
                    ...
                    {
                        _logger.LogWarning($"Failed to enhance job {jobs[i].Title}: {ex.Message}");
            ...
            if (jobs.Count == 0)
            {
                _logger.LogWarning("No jobs extracted from Glassdoor email");
                _logger.LogDebug($"Email subject: {subject}");
                ...
                _logger.LogInformation($"Job-related keywords found: {string.Join(", ", foundKeywords)}");
            }
            
            _logger.LogInformation($"Successfully parsed {jobs.Count} jobs from Glassdoor email alert");
            return jobs;
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error parsing Glassdoor email: {ex.Message}");
            return jobs;
        }
    }
    ...
    private async Task<List<EnhancedJobListing>> ParseDiceJobAlert(string subject, string body, DateTime emailDate)
    {
        ...
        {
            _logger.LogWarning($"Error parsing Dice email: {ex.Message}");
            return jobs;
        }
    }
    ...
    private async Task<List<EnhancedJobListing>> ParseIndeedJobAlert(string subject, string body, DateTime emailDate)
    {
        ...
        {
            _logger.LogWarning($"Error parsing Indeed email: {ex.Message}");
            return jobs;
        }
    }
    ...
    public async Task<List<EnhancedJobListing>> EnhanceJobsWithDetails(List<EnhancedJobListing> jobs)
    {
        ...
        foreach (EnhancedJobListing job in jobs)
        {
            ...
            {
                _logger.LogWarning($"Failed to enhance job details for {job.Title}: {ex.Message}");
        ...
        try
        {
            ...
            foreach (string url in urlsToTry)
            {
                try
                {
                    ...
                    workingUrl = url;
                    _logger.LogInformation($"Successfully accessed job page: {url}");
                    break;
                }
                catch (Exception ex)
                {
            ...
            {
                _logger.LogWarning($"All URL attempts failed for job ID {jobId}");
            ...
            foreach (string pattern in companyPatterns)
            {
                ...
                if (match.Success)
                {
                    ...
                    _logger.LogDebug($"Extracted company: {job.Company}");
                    break;
                }
            }
            ...
            foreach (string pattern in locationPatterns)
            {
                ...
                if (match.Success)
                {
                    ...
                    job.IsRemote = DetermineRemoteStatus(job.Location, job.Title);
                    _logger.LogDebug($"Extracted location: {job.Location}");
                    break;
                }
            }
            ...
            foreach (string pattern in salaryPatterns)
            {
                ...
                if (match.Success)
                {
                    ...
                    _logger.LogDebug($"Extracted salary: {job.Salary}");
                    break;
                }
            }
            ...
            _logger.LogInformation($"Successfully enhanced job: {job.Title} at {job.Company} in {job.Location} (Score: {job.MatchScore})");
            
            return job;
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error scraping Glassdoor job details: {ex.Message}");
