...
[McpServerToolType]
public class TerminalTools(ProcessManager processManager, SecurityManager securityManager, AuditLogger auditLogger)
{
    [McpServerTool]
        ...
        [Description("Working directory path")] string? workingDirectory = null)
    {
        if (securityManager.IsCommandBlocked(command))
        {
            ...
            auditLogger.LogCommandExecution(command, false, error);
            return error;
        }

        if (workingDirectory != null && !securityManager.IsDirectoryAllowed(workingDirectory))
        {
        ...
        [Description("Input to send to the process")] string input)
    {
        ...
        if (success)
        {
            auditLogger.LogOperation("Send_Input", $"Session: {sessionId}, Input: {input}", true);
            return $"Input sent successfully to session {sessionId}";
        }

        auditLogger.LogOperation("Send_Input", $"Session: {sessionId}, Input: {input}", false);
