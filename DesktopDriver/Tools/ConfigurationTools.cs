...
public class ConfigurationTools(SecurityManager securityManager, AuditLogger auditLogger)
{
    ...
    public string GetConfiguration()
    {
        try
        {
            ...
            {
                foreach (string dir in securityManager.AllowedDirectories)
                {
            ...
            {
                foreach (string cmd in securityManager.BlockedCommands.OrderBy(x => x))
                {
        ...
        [Description("Directory path to test")] string directoryPath)
    {
        ...
        {
            string fullPath = Path.GetFullPath(directoryPath);
            bool isAllowed = securityManager.IsDirectoryAllowed(fullPath);
        ...
        [Description("Command to test")] string command)
    {
        try
        {
            bool isBlocked = securityManager.IsCommandBlocked(command);
