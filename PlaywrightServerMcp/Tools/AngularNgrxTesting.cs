...
public class AngularNgrxTesting(PlaywrightSessionManager sessionManager)
{
    ...
    [McpServerTool]
    [Description("Test NgRx store actions, effects, and state management with comprehensive automation")]
    public async Task<string> TestNgrxStoreActions(
        [Description("Session ID for browser context")] string sessionId = "default",
        [Description("Specific action types to test (comma-separated, optional)")] string actionTypes = "",
        [Description("Test effects along with actions (default: true)")] bool testEffects = true,
        [Description("Test reducers for state mutations (default: true)")] bool testReducers = true,
        [Description("Test selectors for memoization and performance (default: true)")] bool testSelectors = true,
        [Description("Validate state structure and normalization (default: true)")] bool validateStateStructure = true,
        [Description("Generate performance recommendations (default: true)")] bool generateRecommendations = true,
        [Description("Maximum execution time in seconds (default: 60)")] int timeoutSeconds = 60)
    {
        try
        {
