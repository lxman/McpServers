...
[McpServerTool]
        ...
        int maxComponents)
    {
        ...
        try
        {
            ...
            {
                result.BundleSizeDependencies = await AnalyzeDependencyImpacts(directory, packageJsonPath, buildOutputPath);
            }
    ...
    /// </summary>
    private async Task<BundleSizeDependencyAnalysis> AnalyzeDependencyImpacts(string directory, string packageJsonPath, string buildOutputPath)
    {
        return await Task.Run(() =>
        {
            var dependencyAnalysis = new BundleSizeDependencyAnalysis();
    ...
    private async Task<BundleOptimizationRecommendations> GenerateOptimizationRecommendations(BundleSizeAnalysisResult result)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                allRecommendations.AddRange(GenerateDependencyOptimizationRecommendations(result.BundleSizeDependencies));
    ...
    private DependencyImpact AnalyzeDependencyImpact(string name, string version, string buildOutputPath)
    {
        var impact = new DependencyImpact
        {
            ...
            SizeBytes = EstimateDependencySize(name)
        };
    ...
    private DependencyOptimization AnalyzeDependencyOptimization(List<DependencyImpact> dependencies)
    {
        var optimization = new DependencyOptimization
        {
            ...
            QuickWins = [],
            OptimizedDependencies = dependencies.Count(d => d.IsTreeShakable),
            UnusedDependencies = dependencies.Count(d => !d.IsUsed),
            PotentialSavings = dependencies.Where(d => !d.IsUsed).Sum(d => d.SizeBytes)
        };
    ...
    private List<string> GenerateDependencyOptimizationOpportunities(BundleSizeDependencyAnalysis analysis)
    {
        var opportunities = new List<string>();
    ...
    private DependencyOptimizationInfo AnalyzeDependencyOptimizationInfo(string name, long size)
    {
        var info = new DependencyOptimizationInfo
        {
            ...
            CanBeTreeShaken = IsTreeShakableDependency(name),
            CanBeLazyLoaded = !name.StartsWith("@angular/core") && !name.Contains("polyfill"),
            HasSmallerAlternatives = GetDependencyAlternatives(name).Count > 0
        };
    ...
    private List<OptimizationRecommendation> GenerateDependencyOptimizationRecommendations(BundleSizeDependencyAnalysis dependencies)
    {
        var recommendations = new List<OptimizationRecommendation>();
    ...
    private int ParseTimeEstimate(string timeEstimate)
    {
        ...
        {
            MatchCollection numbers = Regex.Matches(timeEstimate, @"\d+");
            if (numbers.Count > 0)
            {
