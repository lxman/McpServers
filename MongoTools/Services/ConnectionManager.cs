...
using ConnectionInfo = MongoTools.Configuration.ConnectionInfo;

namespace MongoTools.Services;
...
public class ConnectionManager : IDisposable
{
    ...
    public async Task<string> AddConnectionAsync(string serverName, string connectionString, string databaseName)
    {
        ...
        try
        {
            ...
            _currentDatabases[serverName] = databaseName; // NEW: Track the current database
    ...
    public string? GetCurrentDatabase(string serverName)
    {
        ...
        return _currentDatabases.GetValueOrDefault(serverName);
    }
    ...
    public IMongoDatabase? GetDatabase(string serverName)
    {
        ...
        return _databases.GetValueOrDefault(serverName);
    }
    ...
    public ConnectionInfo? GetConnectionInfo(string serverName)
    {
        ...
        return _connectionInfo.GetValueOrDefault(serverName);
    }
    ...
    public string GetConnectionsStatus()
    {
        var connections = _connectionInfo.Values.Select(info => new
        {
            info.ServerName,
            info.DatabaseName,
            CurrentDatabase = GetCurrentDatabase(info.ServerName), // NEW: Show the current database
            info.ConnectedAt,
            info.LastPing,
            info.IsHealthy,
    ...
    public void CleanupConnections()
    {
        ...
        foreach (string serverName in serverNames)
        {
            if (!_connectionInfo.TryGetValue(serverName, out ConnectionInfo? info) || info.IsHealthy) continue;
